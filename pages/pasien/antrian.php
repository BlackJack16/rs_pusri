<section class="content-header">
   <h1><small></small></h1>
    <ol class="breadcrumb">
		<li><a href="#"><i class="fa fa-th"></i>Antrian Anak</a></li>
    </ol>
</section>
<?php
	include "dist/koneksi.php";

	$DataAntrian =mysql_query("SELECT * FROM `antrianPasien` inner join pasien on antrianPasien.NIK=pasien.NIK inner join dokter on antrianPasien.idDokter=dokter.idDokter WHERE status = 0 and tglAntrian = CURDATE() and dokter.spesialis='Anak' order by antrianPasien.noAntrian asc limit 1");
	...
	$DataAntrianAll =mysql_query("SELECT * FROM `antrianPasien` inner join pasien on antrianPasien.NIK=pasien.NIK inner join dokter on antrianPasien.idDokter=dokter.idDokter WHERE tglAntrian = CURDATE() and status=0 and dokter.spesialis='Anak'");
...
// $DataDokter =mysql_query("SELECT antrianPasien.noAntrian,`antrianPasien`.NIK as NIK, `pasien`.nama as namaPasien, `dokter`.nama as namaDokter, dokter.spesialis as spesialis, jadwal.kamar FROM `antrianPasien` INNER JOIN dokter ON dokter.idDokter = antrianPasien.idDokter INNER JOIN pasien on pasien.NIK = antrianPasien.NIK inner join jadwal on antrianPasien.idDokter=jadwal.idDokter WHERE tglAntrian = CURDATE() and NIKPendaftar = $_SESSION[username] order by antrianPasien.idDokter desc");
		...
		$DataDokter =mysql_query("SELECT antrianPasien.noAntrian,`antrianPasien`.NIK as NIK, `pasien`.nama as namaPasien, `dokter`.nama as namaDokter, dokter.spesialis as spesialis FROM `antrianPasien` INNER JOIN dokter ON dokter.idDokter = antrianPasien.idDokter INNER JOIN pasien on pasien.NIK = antrianPasien.NIK WHERE tglAntrian = CURDATE() and NIKPendaftar = $_SESSION[username] and dokter.spesialis='Anak' order by antrianPasien.noAntrian asc");
